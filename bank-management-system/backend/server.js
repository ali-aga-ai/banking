...
app.post('/api/customers', (req, res) => {
  const { firstName, middleName, lastName, creditScore, address, contactNumber, password } = req.body;

  const insertQuery = 'INSERT INTO customer (f_name, m_name, l_name, credit_score, address, contact_number, password) VALUES (?, ?, ?, ?, ?, ?, ?)';
  pool.query(insertQuery, [firstName, middleName, lastName, creditScore, address, contactNumber, password], (err, insertResult) => {
      if (err) {
...
app.post('/api/accounts', (req, res) => {
  const { f_name, password, accountNo, balance, accType, interestRate } = req.body;
  
  // Query to validate customer credentials
  const validateQuery = 'SELECT customer_id FROM customer WHERE f_name = ? AND password = ?';
  pool.query(validateQuery, [f_name, password], (validateErr, validateResult) => {
      if (validateErr) {
          console.error('Error validating customer credentials:', validateErr);
          return res.status(500).json({ error: 'Failed to validate customer credentials' });
      }
      ...
      const checkAccountQuery = 'SELECT * FROM account WHERE customer_id = ? AND acc_type = ?';
      pool.query(checkAccountQuery, [customerId, accType], (checkErr, checkResult) => {
          if (checkErr) {
          ...
          if (checkResult.length > 0) {
              ...
              return res.status(409).json({ error: 'Account of the same type already exists' });
          }

          // If no existing account of the same type, proceed to insert into the account table
          const insertAccountQuery = 'INSERT INTO account (account_no, customer_id, balance, acc_type, interest_rate) VALUES (?, ?, ?, ?, ?)';
          pool.query(insertAccountQuery, [accountNo, customerId, balance, accType, interestRate], (err, result) => {
              if (err) {
...
app.post('/api/cards', (req, res) => {
  ...
  pool.query(customerIdQuery, [f_name], (customerIdErr, customerIdResult) => {
    ...
    const accountNoQuery = 'SELECT account_no, acc_type FROM account WHERE customer_id = ? LIMIT 1';
    pool.query(accountNoQuery, [customerId], (accountNoErr, accountNoResult) => {
      if (accountNoErr) {
        console.error('Error finding account number:', accountNoErr);
        return res.status(500).json({ error: 'Failed to find account number' });
      }

      if (accountNoResult.length === 0) {
        return res.status(404).json({ error: 'No account found for the customer' });
      }

      const { acc_no, acc_type } = accountNoResult[0];

      // Insert the card details into the database
      const insertQuery = 'INSERT INTO card (card_no, customer_id, acc_no, card_type) VALUES (?, ?, ?, ?)';
      pool.query(insertQuery, [cardNo, customerId, acc_no, cardType ], (insertErr, insertResult) => {
        if (insertErr) {
...
app.post('/api/loans', (req, res) => {
  const { username, password, amount, collateral, interest, timeMonths } = req.body;
